...
type UnknownToCell = (record: unknown, col: number) => GridCell;
type UnknownOnCellEdited = (record: unknown, col: number, value: GridCell) => void;

function createRowCaches(records: readonly unknown[], columns: readonly GridColumn[], toCell: UnknownToCell) {
    return records.map(record =>
        createCache((): readonly GridCell[] => {
            const cells = new Array<GridCell>(columns.length);
...
type RowCache = ReturnType<typeof createRowCaches>[number];

interface CacheEntry {
    // Everything the cached closures capture is here or is the WeakMap key (`records`):
    //   - the row caches capture `records[i]`, `columns`, `toCell`
    //   - `getCellContent` captures `projections`
    //   - `onCellsEdited` captures `records` and `onCellEdited`
    readonly columns: readonly GridColumn[];
    readonly toCell: UnknownToCell;
    readonly onCellEdited: UnknownOnCellEdited | undefined;
    readonly caches: readonly RowCache[];
    readonly projections: readonly (readonly GridCell[])[];
    readonly result: RecordsSourceResult;
}
...
function sameProjections(a: readonly (readonly GridCell[])[], b: readonly (readonly GridCell[])[]): boolean {
...
export function recordsSource<T extends object>(p: RecordsSourceProps<T>): RecordsSourceResult {
    const { records, columns } = p;
    // Safe by construction: the only values ever passed to these are elements of `records` itself.
    const toCell = p.toCell as UnknownToCell;
    const onCellEdited = p.onCellEdited as UnknownOnCellEdited | undefined;
    ...
    const projections = new Array<readonly GridCell[]>(caches.length);
    ...
    const result: RecordsSourceResult = {
        columns,
        rows: records.length,
        // O(1) lookup, no computation: everything was projected above.
        getCellContent: ([col, row]: Item): GridCell => projections[row]?.[col] ?? FALLBACK_CELL,
        onCellsEdited:
    ...
    cache.set(records, { columns, toCell, onCellEdited, caches, projections, result });
    return result;
}
