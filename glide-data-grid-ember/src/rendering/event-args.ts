import type { Item, Rectangle, CellActivationBehavior } from "./data-grid-types.ts";
...
export interface GridKeyEventArgs {
    ...
    // Phase 1 note: source typed this as `React.KeyboardEvent<HTMLElement> | undefined`. The render
    // engine never inspects the raw event, it is only plumbed through for consumers, so a framework-
    // neutral `KeyboardEvent` (the DOM type) is used instead. Ember's event wiring is a later phase.
    readonly rawEvent: KeyboardEvent | undefined;
    readonly location: Item | undefined;
}

/** @category Types */
export type GridMouseEventArgs =
    GridMouseCellEventArgs | GridMouseHeaderEventArgs | GridMouseOutOfBoundsEventArgs | GridMouseGroupHeaderEventArgs;
