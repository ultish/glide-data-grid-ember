...
import { getMiddleCenterBias, roundedRect } from "../render/data-grid-lib.ts";
import { interpolateColors } from "../color-parser.ts";
...
function isButtonCell(cell: CustomCell): cell is ButtonCell {
    return (cell.data as { kind?: unknown }).kind === "button-cell";
}

function unpackColor(color: PackedColor, theme: Record<string, unknown>, hoverAmount: number): string {
...
function getButtonInterior(bounds: Rectangle, theme: FullTheme): Rectangle {
    return {
...
export const buttonCellRenderer: CustomRenderer<ButtonCell> = {
    ...
    draw: (args, cell) => {
        const { ctx, theme, rect, hoverX, hoverY, frameTime, drawState, requestAnimationFrame, overrideCursor } = args;
        const { title, backgroundColor, color, borderColor, borderRadius } = cell.data;

        const { x, y, width, height } = getButtonInterior(rect, theme);
        if (width <= 0 || height <= 0) return;

        const isHovered = getIsHovered(rect, hoverX, hoverY, theme);

        // eslint-disable-next-line prefer-const
        let [state, setState] = drawState as [ButtonDrawState | undefined, (state: ButtonDrawState) => void];
