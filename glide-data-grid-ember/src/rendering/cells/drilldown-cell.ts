// Ported from `packages/core/src/cells/drilldown-cell.tsx` (Phase 4c). Draws a horizontal row of
// chips, each optionally preceded by a small square image/icon loaded via the already-ported
// `ImageWindowLoader` (`args.imageLoader.loadOrGetImage`, Phase 1 -- see
// `image-window-loader-interface.ts`/`common/image-window-loader.ts`), using the same
// `roundedRect`/`measureTextCached`/`getMiddleCenterBias`/`getEmHeight` primitives as bubble-cell.
//
// Like bubble-cell, source's `provideEditor` (`drilldown-overlay-editor.tsx`) is a plain,
// non-interactive display of the full chip list (no `onChange`/`onFinishedEditing` wiring at all)
// plus a decorative, permanently-hidden `<textarea>`. `onPaste` unconditionally returns
// `undefined`. Functionally read-only end to end -- per PORTING-NOTES.md's Phase 4c scope, no
// overlay editor is built here, confirmed by reading source directly.
import { GridCellKind, type DrilldownCell, type DrilldownCellData } from "../data-grid-types.ts";
import type { BaseDrawArgs, InternalCellRenderer } from "../cell-types.ts";
import { getEmHeight, getMiddleCenterBias, measureTextCached, roundedRect } from "../render/data-grid-lib.ts";
...
function getAndCacheDrilldownBorder(
    ...
    const cached = drilldownCache[key];
    if (cached !== undefined) {
        return {
            el: cached,
            height: innerHeight,
