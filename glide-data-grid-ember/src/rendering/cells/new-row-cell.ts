// Ported near-verbatim from `packages/core/src/cells/new-row-cell.tsx` (Phase 4d) -- the
// hover-fade "+" affordance drawn for the trailing blank row (`showTrailingBlankRow`, see
// `grid-host-controller.ts`). `InnerGridCellKind.NewRow` is an `InnerOnlyGridCell` kind (already
// ported in Phase 1's `data-grid-types.ts`, see `isInnerOnlyCell`) -- it has no `provideEditor`,
// `allowOverlay: false`, and is never a real `GridCell` a consumer's `getCellContent` returns for
// an actual row; `GridHostController` synthesizes it only for the one virtual trailing row.
import { InnerGridCellKind, type NewRowCell } from "../data-grid-types.ts";
