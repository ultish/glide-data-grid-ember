...
import type { CellActivationBehavior, Item } from "./data-grid-types.ts";
 ...
 */
export function isValidClick(down: Item | undefined, up: Item, isTouch = false): boolean {
...
export interface ClickActivationArgs {
    readonly behavior: CellActivationBehavior;
    /** `MouseEvent.detail >= 2` for the mouseup ending a double-click. */
    readonly isDoubleClick: boolean;
    /** The cell the mouseup landed on. */
    readonly location: Item;
    /** The active cell *now*, i.e. after the mousedown's selection ran. */
    readonly currentCell: Item | undefined;
    /** The active cell *before* the mousedown. Source keeps this in `mouseState.previousSelection`. */
    readonly previousCell: Item | undefined;
}
 ...
 */
export function shouldActivateOnClick(args: ClickActivationArgs): boolean {
    const { behavior, isDoubleClick, location, currentCell, previousCell } = args;
    switch (behavior) {
        case "single-click":
            return true;
        case "double-click":
        case "second-click": {
            ...
            const isClickOnSelected =
                location[0] === currentCell[0] &&
                location[0] === previousCell[0] &&
                location[1] === currentCell[1] &&
                location[1] === previousCell[1];
 ...
 */
export function resolvePointerActivation(
    behavior: CellActivationBehavior,
