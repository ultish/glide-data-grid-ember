...
import { isGroupEqual } from "./render/data-grid-lib.ts";
 ...
 */
export function computeGroupHeaderSpan(
    mappedColumns: readonly GroupedColumnLike[],
    col: number,
 ...
 */
export function computeGroupHeaderSelection(input: GroupHeaderSelectionInput): GroupHeaderSelectionUpdate | undefined {
    const { mappedColumns, col, rowMarkerOffset, selectedColumns, columnSelect, columnSelectionMode } = input;
    ...
    if (input.isMultiKey || input.isTouch === true || columnSelectionMode === "multi") {
        if (selectedColumns.hasAll(span)) {
            ...
            return { newColumns: newVal, append: undefined, span };
        }
        return { newColumns: undefined, append: span, span };
    }
    return { newColumns: CompactSelection.fromSingleSelection(span), append: undefined, span };
}
