...
export default Route(
    <template>
        <header class="gdg-cookbook__intro">
            ...
            <p>
                Basics to advanced, in order. Every chapter that can show a running grid does, and
                the file under it is the complete source — imports, module-scope constants, the
                class, the template. Ember 6 and 7, <code>.gts</code>, no <code>@action</code>.
            </p>
        </header>
        ...
        <p class="gdg-cookbook__note">
            ...
            <em>function</em>, then the engine calls it later, at paint time — so a closure that
            reads
            <code>@tracked</code>
            state lazily never registers a dependency. No error, no warning, no console message. The
            grid looks completely correct until your data changes and nothing happens.
