...
export default class FirstGridChapter extends Component {
    ...
    <template>
        <p>
            Ember 6 or 7, a
            <code>.gts</code>
            file, nothing else to wire. The addon imports its own CSS, registers its own cell types,
            and exposes one component.
        </p>
        ...
        <p class="gdg-cookbook__note">
            <strong>The grid sizes itself to its container and has no
                ...
                args.</strong>
            A container with no height renders a zero-height grid. That is, by a wide margin, the
            most common "nothing appears". Check this before anything else.
        </p>
        ...
        <ul>
            ...
            <li>
                <strong><code>getCellContent</code> is called by the paint loop</strong>, once per
                painted cell. What you are allowed to do inside it is not a style question.
            </li>
            <li>
                <strong><code>PEOPLE</code> is a module constant, so nothing can change.</strong>
                Making it change correctly is the part of this addon that has no error message when
                you get it wrong.
            </li>
        </ul>
    </template>
