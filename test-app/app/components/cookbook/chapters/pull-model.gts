...
getCellContent = ([col, row]: Item): GridCell => this.cells[row]?.[col] ?? BLANK;`;
    ...
    <template>
        <p>
            ...
            <code>@rows</code>
            (a count) and
            <code>@getCellContent</code>, and it asks for cells as it paints them — only the cells
            actually on screen, only when it is drawing them. It never receives your array, never
            iterates it, and never holds a copy. That is the whole reason 200,000 rows costs about
            what 20 does: the viewport is what is expensive, and the viewport is a constant.
        </p>

        <p>
            It also means the grid has no idea when your data changes. Nothing observes
            <code>PEOPLE</code>. The next chapter is entirely about closing that loop; this one is
        ...
        <p>
            ...
            It runs
            <em>inside the draw loop</em>. A full repaint of an ordinary viewport is a few hundred
            calls; a fast scroll is a fresh strip of them every frame; a drag-selection re-reads on
            every mouse move. Anything you do in there, you do at frame rate.
        </p>
        ...
        <p>
            ...
            exists to produce that second version. The next two chapters show why you cannot just
            drop a
            <code>@tracked</code>
            read into
        ...
        <p>
            ...
            <strong>your</strong>
            coordinate space. Row markers, frozen columns, the trailing blank row and the header are
            the grid's own business — it never shifts your indices to account for them, and every
            callback that hands you a column index has already stripped the row-marker column back
            out.
        </p>
        ...
        <p class="gdg-cookbook__note">
            ...
            must return
            <em>something</em>
            for them. Return a blank cell rather than throwing — a throw inside the draw loop takes
            the frame with it.
        </p>
    </template>
}
