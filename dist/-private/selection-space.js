import { CompactSelection } from '../rendering/data-grid-types.js';
 ...
 */
function asConsumerSelection(selection) {
  return selection;
}
const EMPTY_SELECTION = asConsumerSelection({
  current: undefined,
 ...
 */
class MangledSelectionCache {
  entry;
  get(selection, rowMarkerOffset) {
    ...
    const value = shiftSelection(selection, rowMarkerOffset);
    this.entry = {
      src: selection,
      offset: rowMarkerOffset,
      value
    };
    return value;
  }
 ...
 * all.
 */
function mangleSelection(selection, rowMarkerOffset) {
  return shiftSelection(selection, rowMarkerOffset);
}

/** Mangled -> consumer. The inverse of `MangledSelectionCache.get`, and the only way back. */
function unmangleSelection(selection, rowMarkerOffset) {
 ...
 */
function unmangleColumn(mangledCol, rowMarkerOffset) {
  return mangledCol - rowMarkerOffset;
}

export { EMPTY_SELECTION, MangledSelectionCache, asConsumerSelection, mangleSelection, shiftSelection, unmangleColumn, unmangleSelection };
//# sourceMappingURL=selection-space.js.map
