import { mapColumns } from '../rendering/render/data-grid-lib.js';
...
const ROW_MARKER_CHECKBOX_STYLE = "square";
/**
 ...
 */
class MangledLayoutCache {
  entry;
  get(columns, marker, freezeColumns) {
    const cached = this.entry;
    if (cached !== undefined && cached.columns === columns && cached.freezeColumns === freezeColumns && cached.hasMarker === (marker !== undefined) && cached.markerWidth === marker?.width && cached.markerChecked === marker?.checked && cached.markerHeaderDisabled === marker?.headerDisabled && cached.markerThemeOverride === marker?.themeOverride) {
      return cached.value;
    }
    const mangledColumns = marker === undefined ? columns : [{
      title: "",
      width: marker.width,
      hasMenu: false,
      style: "normal",
...
export { MangledLayoutCache };
//# sourceMappingURL=mangled-layout.js.map
