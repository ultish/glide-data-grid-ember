 ...
 */
function resolveOrder(columns, keys) {
  const ranks = new Map();
...
function isIdentityPermutation(map) {
...
function withMovableColumns(p) {
  ...
  if (readReusable && cached.getCellContentIn === getCellContentIn && cached.onCellsEditedIn === onCellsEditedIn && cached.onColumnMovedIn === onColumnMovedIn && cached.onOrderChange === onOrderChange) {
    return cached.result;
  }
  let displayedColumns;
  let displayToOriginal;
  let getCellContent;
  let getOriginalColumnIndex;
  if (readReusable) {
    ...
    getCellContent = cached.getCellContentIn === getCellContentIn ? cached.getCellContent : map === undefined ? getCellContentIn : ([col, row]) => getCellContentIn([map[col] ?? col, row]);
  } else {
    ...
    if (isIdentityPermutation(map)) {
      ...
      getCellContent = ([col, row]) => getCellContentIn([map[col] ?? col, row]);
      getOriginalColumnIndex = index => map[index] ?? index;
    }
  }
  const map = displayToOriginal;
...
export { columnOrderKey, withMovableColumns };
//# sourceMappingURL=movable-columns.js.map
