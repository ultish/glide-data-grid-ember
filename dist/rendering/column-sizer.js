...
function measureCell(ctx, cell, theme, getCellRenderer) {
  ...
  return renderer?.measure?.(ctx, cell, theme) ?? DEFAULT_COLUMN_WIDTH;
}
/**
 ...
 */
function headerAffordanceWidth(column, theme) {
  const iconAllowance = column.icon === undefined ? 0 : 28;
  const indicatorAllowance = column.indicatorIcon === undefined ? 0 : theme.headerIconSize + theme.cellHorizontalPadding;
 ...
 */
function measureColumn(ctx, theme, column, colIndex, sample, getCellRenderer, options) {
  let max = 0;
  ...
  max = Math.max(max, ctx.measureText(column.title).width + theme.cellHorizontalPadding * 2 + headerAffordanceWidth(column, theme));
 ...
 */
function sizeColumns(columns, ctx, theme, sample, getCellRenderer, options) {
  const previousFont = ctx?.font;
  ...
  try {
    return columns.map((c, i) => {
      ...
      return {
        ...c,
        width: measureColumn(ctx, theme, c, i, sample, getCellRenderer, options)
      };
    });
  } finally {
 ...
 */
function applyColumnGrow(columns, clientWidth) {
  let totalWidth = 0;
  let totalGrow = 0;
  const distribute = [];
...
export { DEFAULT_COLUMN_WIDTH, applyColumnGrow, measureCell, measureColumn, sizeColumns };
//# sourceMappingURL=column-sizer.js.map
