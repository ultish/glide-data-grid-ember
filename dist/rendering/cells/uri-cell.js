...
import { prepTextCell, measureTextCached, getMiddleCenterBias, drawTextCell, getMeasuredTextCache } from '../render/data-grid-lib.js';
import { pointInRect } from '../common/math.js';
import { GrowingEntry } from '../../-private/growing-entry.js';
import { createEditPencilIcon } from '../../-private/edit-icons.js';
...
const uriCellRenderer = {
  ...
  onDelete: c => ({
    ...c,
    data: "",
    displayData: c.displayData === undefined ? undefined : ""
  }),
  provideEditor: () => p => buildUriEditor(p),
  onPaste: (toPaste, cell, details) => toPaste === cell.data ? undefined : {
    ...cell,
    data: toPaste,
    displayData: details.formattedString ?? cell.displayData
  }
};
...
function buildUriEditor(p) {
  ...
  return {
    element: container,
    focus: () => {
      if (growingEntry !== undefined) growingEntry.focus();else focusTarget?.focus();
    },
    destroy: () => {
