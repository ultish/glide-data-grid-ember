...
import { getStickyWidth, getFreezeTrailingHeight, computeBounds } from './data-grid-lib.js';
import { blend, withAlpha } from '../color-parser.js';
import { splitRectIntoRegions, hugRectToTarget, intersectRect, rectContains } from '../common/math.js';
import { walkColumns, walkRowsInCol, getSpanBounds } from './data-grid-render.walk.js';
...
function drawHighlightRings(ctx, width, height, cellXOffset, cellYOffset, translateX, translateY, mappedColumns, freezeColumns, headerHeight, groupHeaderHeight, rowHeight, freezeTrailingRows, rows, allHighlightRegions, theme) {
  ...
  const drawCb = () => {
    ...
    for (const dr of drawRects) {
      for (const s of dr) {
        if (s?.rect !== undefined && intersectRect(0, 0, width, height, s.rect.x, s.rect.y, s.rect.width, s.rect.height)) {
...
export { drawColumnResizeOutline, drawFillHandle, drawHighlightRings };
