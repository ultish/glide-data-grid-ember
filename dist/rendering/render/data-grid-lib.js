...
import { clearCache, split } from 'canvas-hypertxt';
...
function mapColumns(columns, freezeColumns) {
...
function drawMultiLineText(ctx, data, x, y, w, h, bias, theme, contentAlign, hyperWrapping) {
  ...
  const split$1 = split(ctx, data, fontStyle, w - theme.cellHorizontalPadding * 2, hyperWrapping ?? false);
  const emHeight = getEmHeight(ctx, fontStyle);
  const lineHeight = theme.lineHeight * emHeight;
  const actualHeight = emHeight + lineHeight * (split$1.length - 1);
...
export { cellIsInRange, cellIsSelected, computeBounds, drawLastUpdateUnderlay, drawMenuDots, drawTextCell, drawTextCellExternal, getColumnIndexForX, getEffectiveColumns, getEmHeight, getFreezeTrailingHeight, getMeasuredTextCache, getMiddleCenterBias, getRowIndexForY, getStickyWidth, gridSelectionHasItem, isGroupEqual, itemIsInRect, itemsAreEqual, mapColumns, measureTextCached, prepTextCell, rectBottomRight, remapForDnDState, roundedPoly, roundedRect };
