import { pointInRect, intersectRect } from '../common/math.js';
...
import { getMeasuredTextCache, measureTextCached, getMiddleCenterBias, roundedPoly, drawMenuDots } from './data-grid-lib.js';
import { getActionBoundsForGroup } from './group-header-actions.js';
...
function computeHeaderLayout(ctx, c, x, y, width, height, theme, isRTL) {
  ...
  if (c.indicatorIcon !== undefined) {
    const textWidth = ctx === undefined ? getMeasuredTextCache(c.title, theme.headerFontFull)?.width ?? 0 : measureTextCached(c.title, ctx, theme.headerFontFull).width;
...
function drawHeader(ctx, x, y, width, height, c, selected, theme, isHovered, posX, posY, hasSelectedCell, hoverAmount, spriteManager, drawHeaderCallback, touchMode) {
  ...
  if (drawHeaderCallback !== undefined) {
    drawHeaderCallback({
      ...
      hasSelectedCell,
      spriteManager,
      menuBounds: headerLayout?.menuBounds ?? {
        x: 0,
        y: 0,
        height: 0,
        width: 0
...
export { computeHeaderLayout, drawGridHeaders, drawGroups, drawHeader };
