...
| **`experimental` bag, rest** | PARTLY DONE (2026-08-12) | Flattened into real args rather than an `experimental` bag, following 2.5's precedent with `hyperWrapping`. **Done:** `@disableMinimumCellWidth`, `@renderStrategy`, `@enableFirefoxRescaling` / `@enableSafariRescaling` (the scroll-time DPR cap, 200ms settle). **Won't port:** `scrollbarWidthOverride` (its only use upstream is the `idealWidth`/`idealHeight` sizing helper this port does not have — the port measures the live element for its one scrollbar hit-test), `kineticScrollPerfHack` (touch, 9c), `isSubGrid` (a className for source's click-outside library), `disableAccessibilityTree` (9b). **Still open:** the two below. |
| **`experimental.strict`** | `M` | Returns a Loading cell for anything outside the last reported visible region (plus the selected cell and any freeze region) — a correctness harness that makes a consumer notice when the grid asks for rows they have not loaded. Needs the controller to keep its own visible region, which `onVisibleRegionChanged` already computes. Source: `data-editor.tsx:1350-1372`. |
| **`experimental.eventTarget`** | `S`–`M` | Lets a consumer redirect the grid's *window-level* listeners (paste/copy/cut, and 9h's window `mousemove`) at another target, for a grid living in an iframe or a portal. Mechanical but touches every `window.addEventListener` in the controller, so it wants one pass and a browser check. |
