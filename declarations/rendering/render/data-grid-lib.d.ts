...
import { type Item, type GridSelection, type InnerGridCell, type InnerGridColumn, type Rectangle, type BaseGridCell } from "../data-grid-types.ts";
import type { BaseDrawArgs, PrepResult } from "../cell-types.ts";
import type { FullyDefined } from "../common/support.ts";
export interface MappedGridColumn extends FullyDefined<InnerGridColumn> {
    sourceIndex: number;
    sticky: boolean;
}
export declare function mapColumns(columns: readonly InnerGridColumn[], freezeColumns: number): readonly MappedGridColumn[];
